<template>
  <div class="hello">
    ...
    <p>{{ count.toFixed(2) }}</p>
    <p>{{ str.toUpperCase() }}</p>
    <!-- <p v-bind:class="{{ color }}">{{ str.toUpperCase() }}</p> -->
    <p v-bind:class="color" v-bind:style="s">{{ str.toLowerCase() }}</p>
    <!-- 插值表达式不能应用在设置属性上 -->
    <ul v-html="li"></ul>
    <div>{{ danger }}</div>
    <div>
      <input type="radio" name="sex" @click="changeSex(1)" :checked="danger==1"/>男
...
export default {
  ...
  data() {
    return {
      ...
      count: 0,
      msg: "当前页面是组件页面，项目是通过vue create项目名称创建的",
      li: "<li>这是一个li标签</li>",
      danger: "1",
      cj: 76.5,
      pf: [
...
<!-- 脚手架创建 -->
<!-- vue create项目 脚手架创建 vue/cli vue.js -->
<!-- 手动创建 -->
